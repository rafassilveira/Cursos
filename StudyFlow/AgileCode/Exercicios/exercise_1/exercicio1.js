...
RegExp - Escape
*/
let data = 'create table author(id number, name string, age number, city string, state string, country string)'

let regExp = /table\s(\w+)/g
let regExpColumn = /\((\D+)\)/g
let result = regExp.exec(data)
let resultColum = regExpColumn.exec(data)
let tableName = result[1]
let column = resultColum[1].split(',')

console.log(JSON.stringify(table, undefined, " "));