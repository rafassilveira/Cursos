...
Percorra as colunas com
for / of e utilize a notação de[] tanto para criar e acessar as propriedades nos objetos.É possível utilizar um parâmetro na operação JSON.stringify para formatar o objeto.Para isso, passe como terceiro parâmetro alguns espaços em branco ou o caracter tab.
...
for (let col in column) {
  column[col] = column[col].trim();
}
for (let col in column) {
  column[col] = column[col].replace(" ", ":");
}
